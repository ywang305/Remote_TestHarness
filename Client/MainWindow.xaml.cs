 ...
 * Package Operations:
 * -------------------
 * Send test request msg to test harness
 * Queries into Repository for Logs and Libraries
 * Upload dlls, download logs to/from Repository
 * 
 * Required Files:
 * - Client.cs, ITest.cs, Logger.cs, Message.cs, HiTimer.cs
 * 
 * Maintenance History:
 * --------------------
 * ver 1.1 : 19 Nov 2016
 * - more display details on GUI
 *  
 * ver 1.0 : 16 Nov 2016
 * - first release
 */
...
namespace ClientEnd
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            Console.Title = " client #1 demo";
            client = new Client(rcvThreadProc);

            //--------as a client, sending test request msg-------------------------
            for(int i=0; i<10; i++)
            {
                client.MakeMessage("Fawcett", "testrequest", client.endPoint,
        ...
        void rcvThreadProc()
        {
            ...
            {
                Message msg = null;
                msg = client.comm.rcvr.GetMessage(); // get msg from rcvBlockingQ
    ...
    public class Client
    {
        ...
        public Client(ThreadStart rcvThreadProc)
        {
            ...
            endPoint = CommService<Client>.makeEndPoint("http://localhost", 4039);
